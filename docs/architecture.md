...
- **Driver ETAs are presentation-shifted at map time, not in storage.** The solver stamps arrival times forward from `DepartAt`, which sits well before the arrival window — so persisted ETAs read as the driver arriving far too early and idling. `Mappers.ToDto` slides each route's whole timeline later by a uniform shift, emitting `Departure`/`DestinationArrival` so the driver leaves just-in-time and lands a few minutes before the target. Keeping this in the mapper (rather than rewriting persisted `Stop.EstimatedArrival`) means the stored solution stays a pure solver artefact and the same rows can be re-presented if the arrival target ever changes.
